                ...
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 12) {
                        ZStack {
                            RoundedRectangle(cornerRadius: 24, style: .continuous)
                                ...
                                .shadow(color: Color(hex: "#FF96C8").opacity(0.35), radius: 24, x: 0, y: 8)
                        ...
                        .onAppear {
                            withAnimation(.easeOut(duration: 0.5)) { animateLogo = true }
                        }
                    ...
                    HStack(spacing: 0) {
                        ForEach(["Log In", "Sign Up"], id: \.self) { label in
                            let isSelected = (mode == .login && label == "Log In") || (mode == .register && label == "Sign Up")
                            
                            Button(action: {
                                withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) {
                                    mode = label == "Log In" ? .login : .register
                                }
                            }) {
                                Text(label)
                                    ...
                                    .foregroundColor(isSelected ? .pennyText : .pennySecondaryText)
                                    .clipShape(Capsule())
