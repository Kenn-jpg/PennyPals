...
struct EggShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        path.addEllipse(in: CGRect(x: rect.minX, y: rect.minY + rect.height * 0.1, width: rect.width, height: rect.height * 0.9))
        return path
    }
}
...
struct EggView: View {
    ...
    var body: some View {
        ZStack {
            ...
            Circle().fill(Color(hex: spots)).frame(width: size * 0.15).offset(x: -size * 0.15, y: -size * 0.1)
            Circle().fill(Color(hex: spots)).frame(width: size * 0.2).offset(x: size * 0.15, y: size * 0.15)
        }
        ...
        ZStack {
            ...
            HStack(spacing: size * 0.2) {
                if mood == "hungry" {
                    ...
                    Text("^-^").font(.system(size: size * 0.15, weight: .bold)).foregroundColor(Color(hex: "#2A2440"))
                    Text("^-^").font(.system(size: size * 0.15, weight: .bold)).foregroundColor(Color(hex: "#2A2440"))
                }
            }
            .offset(y: -size * 0.05)
